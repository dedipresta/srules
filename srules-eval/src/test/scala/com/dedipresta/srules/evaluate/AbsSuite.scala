...
final class AbsSuite extends FunSuite {
  ...
  test("fails when abs function is called with no arguments") {
    assertEquals(
      SRules.parse("abs()").flatMap(evaluator.evaluate(_, Map.empty)).isLeft,
      true,
    )
  }

  test("fails when abs function is called with more than one argument") {
    assertEquals(
      SRules.parse("abs(1, 2)").flatMap(evaluator.evaluate(_, Map.empty)).isLeft,
      true,
    )
  }

  test("fails when abs function is called with a non-numeric argument") {
    assertEquals(
      SRules.parse("abs(\"42\")").flatMap(evaluator.evaluate(_, Map.empty)).isLeft,
      true,
    )
  }
