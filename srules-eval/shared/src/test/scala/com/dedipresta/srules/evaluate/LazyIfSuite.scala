...
final class LazyIfSuite extends FunSuite {
  ...
  test("parse and evaluate lazyIf-else expression with 3 arguments (true)") {
    assertEquals(
      Parser.parser.parseAll("lazyIf(true, 42, 0)").flatMap(evaluator.evaluate(_, Map.empty)),
      Right(Expr.RInt(42)),
    )
  }
  ...
  test("parse and evaluate lazyIf-else expression with 3 arguments (false)") {
    assertEquals(
      Parser.parser.parseAll("lazyIf(false, 42, 0)").flatMap(evaluator.evaluate(_, Map.empty)),
      Right(Expr.RInt(0)),
    )
  }
  ...
  test("parse and evaluate lazyIf-else expression with 3 arguments (true computed)") {
    assertEquals(
      Parser.parser.parseAll("lazyIf(1 + 1 > 0, 42, 2)").flatMap(evaluator.evaluate(_, Map.empty)),
      Right(Expr.RInt(42)),
    )
  }
  ...
  test("parse and evaluate lazyIf-elseif-else expression with 5 arguments (elseif true)") {
    assertEquals(
      Parser.parser.parseAll("lazyIf(false, 42, 4==4, 1, 2)").flatMap(evaluator.evaluate(_, Map.empty)),
      Right(Expr.RInt(1)),
    )
  }
  ...
  test("parse and evaluate lazyIf-elseif-else expression with 5 arguments (elseif false)") {
    assertEquals(
      Parser.parser.parseAll("lazyIf(false, 42, 4==5, 1, 2)").flatMap(evaluator.evaluate(_, Map.empty)),
      Right(Expr.RInt(2)),
    )
  }
  ...
  test("parse and evaluate lazyIf-else expression but does not evaluate the value") {
    assertEquals(
      Parser.parser.parseAll("lazyIf(true, 1/0, 0)").flatMap(evaluator.evaluate(_, Map.empty)),
  ...
  test("parse and evaluate lazyIf-elseif-else expression but does not evaluate the following conditions if the first is true") {
    assertEquals(
      Parser.parser.parseAll("lazyIf(true, 1, 1/0>0, 0)").flatMap(evaluator.evaluate(_, Map.empty)),
      Right(Expr.RInt(1)),
    )
  }
  ...
  test("parse and evaluate lazyIf-elseif-else expression with 7 arguments (elseif false)") {
    assertEquals(
      Parser.parser.parseAll("lazyIf(false, 42, false, 1, false, 2, 3)").flatMap(evaluator.evaluate(_, Map.empty)),
      Right(Expr.RInt(3)),
    )
  }
